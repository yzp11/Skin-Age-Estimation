...
def get_LBP_vector(face_image_path,detector,predictor):
...
def skin_feature(image_path):
    predictor_path = "shape_predictor_68_face_landmarks.dat"
    detector = dlib.get_frontal_face_detector()
    predictor = dlib.shape_predictor(predictor_path)
    LBP_vector=get_LBP_vector(image_path,detector,predictor)

    pca = PCA(n_components=2)
    # x=x.reshape(-1,1)
    # 对数据进行降维
    if LBP_vector==[]:
        LBP_vector=np.zeros(6)
        return LBP_vector

    min_length = min(LBP_vector[0].shape[0],LBP_vector[1].shape[0], LBP_vector[2].shape[0])
    LBP_vector[0] = LBP_vector[0][:min_length]  # 切片截取到跟最短列表一样的长度
    LBP_vector[1]= LBP_vector[1][:min_length]
    LBP_vector[2]= LBP_vector[2][:min_length]

    pca.fit(LBP_vector)
    X_pca = pca.fit_transform(LBP_vector).flatten()
    return X_pca